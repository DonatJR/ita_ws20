...
def get_papers(url):
    ...
    try:
        ...
        print(
            "{} could not be fetched due to a Connection Error. Make sure you are connected to Internet. Technical Details given below.\n".format(
                url
            )
        )
        print(str(e))
    ...
    for tag in tags:
        if tag.has_attr("href") and "html" in tag["href"] and "papers" in tag["href"]:
            ...
            try:
                ...
                print(
                    "{} at {} could not be fetched due to a Connection Error. Make sure you are connected to Internet. Technical Details given below.\n".format(
                        pdf_filename, ref
                    )
                )
                print(str(e))
                ...
                print(
                    "{} at {} could not be fetched du to a Timeout Error\n".format(
                        pdf_filename, ref
                    )
                )
                print(str(e))
...
def create_data_file(start_vol_nr, end_vol_nr):
    ...
    """
    with open(
        "data_jmlr_vol{}.json".format(start_vol_nr)
        if start_vol_nr == end_vol_nr
        else "data_jmlr_vol{}-{}.json".format(start_vol_nr, end_vol_nr),
        "w",
    ) as f:
